class Game extends Phaser.Scene {
    ...
    create() {
        ...
        this.physics.add.collider(this.dog, this.lixo, hitLixo, null, this);
        this.physics.add.collider(this.dog, this.sonic, hitSonic, null, this);
        this.physics.add.collider(this.dog, this.enemy, hitEnemy, null, this);
        ...
        function hitLixo (dog, lixo){
            this.scene.start('Lost');
        } function hitSonic (dog, sonic){
            this.scene.start('Lost');
        } function hitEnemy (dog, enemy){
            this.scene.start('Lost');
        }
        ...
        // #02.9: CONFIGURAÇÕES DA CÂMERA (campo de visão do jogador)
    ...
    update() {
        ...
        if (this.enemy.x === 3100){
            ...
            this.enemy.ida = true;
        // se o inimigo estiver na posição x > 1500 & inimiga ida for verdadeiro a velocidade dele irá diminuir -10 no eixo x.
        } if (this.enemy.x > 1500 && this.enemy.ida === true) {
            this.enemy.x -= 10;
        // se o inimigo estiver na posição = 15000 => Não inverter a imagem e deixar a variável ida falsa.
        }if (this.enemy.x === 1500){
            this.enemy.setFlipX(false);
            this.enemy.ida = false;
        // se o inimigo estiver na posição x < 3100 & inimiga ida for falso a velocidade dele irá aumentar +10 no eixo x.
